...
### Флуд
*Рекомендуемая мера наказания: мут от 1 ч. до перманентного Бана. (в зависимости от тяжести).*
...
*Если осуществляется *Рейд+Флуд/Спам атака*, всех причастных вы перманентно баните, а потом оставляете отчёт в стафф-чате. С парой скринов из чата.*
...
**Важно:** **нельзя** перманентно банить за 1–2 оскорбления. Бан возможен только в случае, если пользователь продолжительное время ведёт себя агрессивно и имеет множество мутов и варнов за плечами.
...
### Распространение платных ресурсов без согласия авторов.
Распространения платного контента (например, скачанными платными картами, ресурспаками, модами) без разрешения правообладателей.
*Рекомендуемая мера наказания: удалять сообщение + условное предупреждение*
Рецедив: *мут на 1 дн.*
...
- Если пользователь использует другие языки для оскорблений, мата или нарушения правил, выдавайте более суровое наказание (мут на 1 дн.) + варн на 7 дней.
...
На первый раз — условное предупреждение. Если продолжается — мут по *Рекомендуемой мере* выше + варн на 7 дн.  
...
-# Иногда эта функция иногда отключается администрацией в настройках бота, однако это не ваша забота.
...
**Рецедив нарушения:** мут на 7 дн.  

### Разглашение личных данных (фото, паспорта)
*Рекомендуемая мера наказания: от мута навсегда до перманентного бана*
...
*Рекомендуемая мера наказания: от мута на 7 дн. до перманентного бана.* +Удалите сообщения нарушителя.
...
Если пользователь игнорирует ваше предупреждение, выдаёте мут на 30 мин. - 60 мин.
...
Рассматривать апелляцию должен модератор/администратор, **не выдававший** наказание. Должен запросить у пользователя:
- Причину наказания.
- Скриншот нарушения.
...
После получения информации **администратор** или Нао, не участвовавший в выдаче наказания, рассматривает апелляцию. Наказавший администратор/модератор может предоставить дополнительные материалы и обсудить ситуацию в стафф-чате.
...
- **Наказание было обосновано выдано:** Обжалование отклоняется. (Сообщаете в тикете, что мера наказания была обоснована, а обжалование отклонено)

## Вознаграждение опытом. (Staff)
- Администрации & Модерации NCS и NCLDS получает голую ставку 2500exp/месяц
-# Администрация может получать незначительные надбавки.
-# Если администратор/модератор малоактивит, он получает пониженную ставку.
- За сверхурочную/дополнительную активности может начисляться дополнительно exp.

**Опыт начисляет один из администраторов**
-# // Пока что это VelDan X

## Вознаграждение опытом. (Users)
- За каждый попавший мод из предложки начисляется **+1000 exp** участнику 
-# +Дополнительный Exp может быть начислен в зависимости от таких факторов: Новизны мода (Мало где успели упоминуть), значительная востребованость, революционность+Качество проработки мода. Простыми словами: За предложенный хороший мод, который ещё не успел зайхайпить в РУ-сегменте можно накинуть дополнительно опыта.
- За решение проблемы в форуме Помощи начисляется от **250 - 3000 exp** (В зависимости от тяжести самой проблемы и результата помощи )

- **Доп. EXP:** Может быть начислено ещё за создание мода для сервера (по запросу администрации NCS), за создание мода, рп, датапака (+Если закинул в предложку и попал в видос).
-# Простыми словами у нас действует программа поддержки разработчиков

**Опыт начисляет один из администраторов**
-# // Пока что это VelDan X

## FAQ
- Q:*Бот для модерации перестал работать. Что делать?*      
A: Выдайте мут вручную. Как показано на скриншоте:      
![image](https://github.com/VelDanXx/manual-nclds/blob/main/reference/image2.png)

- Q: *Helper не работает, мне оповестить о проблеме?*        
A: Необязательно, скорее всего Я в курсе проблемы. Сообщайте лишь о багах.

- Q: *Я модератор, но мне недоступны права модератора?*      
A: Проверьте настройки двухфакторной аунтефикации. Она должна быть включена.

- Q: *Мне заблокировали аккаунт, мне нужно перенести должность на другой аккаунт?*        
A: Для таких случаев предусмотрены кодовые фразы (вы должны были написать их велдану либо нао). Вы её получаете в личные сообщения и используете лишь в подобных ситуациях.

- Q: *Что такое флуд?*      
A: Это объёмное сообщение, не несущие смысловой нагрузки.  

- Q: *NCS - это что?*         
A: NCS - это аббревиатура, которая расшифровывается как NaoCraftLabCommunity Server. Тематический Майнкрафт сервер комьюнити дискорд сервера NaoCraftLab.

- Q: *NCLDS - это?*       
A: NCLDS - это аббревиатура, которая расшифровывается как NaoCraftLab Discord Server. Тематический Дискорд сервер Ютубера NaoCraftLab.
